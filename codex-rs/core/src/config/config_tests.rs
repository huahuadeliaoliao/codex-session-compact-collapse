...
#[tokio::test]
async fn loads_compact_strategy_from_config() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cfg = ConfigToml {
        compact_strategy: Some(CompactStrategy::Session),
        ..Default::default()
    };
    ...
    assert_eq!(config.compact_strategy, CompactStrategy::Session);

    Ok(())
}

#[tokio::test]
async fn profile_compact_strategy_overrides_global_strategy() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cfg = ConfigToml {
        compact_strategy: Some(CompactStrategy::Default),
        profile: Some("session".to_string()),
