...
#[test]
fn collapse_mode_default_preserve_turns_is_five() {
    assert_eq!(5, DEFAULT_COMPACT_PRESERVE_TURNS);
}
...
#[test]
fn loads_compact_mode_and_preserve_turns() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;

    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            compact_mode: Some(CompactMode::Collapse),
            compact_preserve_turns: Some(5),
            ..Default::default()
        },
        ConfigOverrides::default(),
        codex_home.path().to_path_buf(),
    ...
    assert_eq!(CompactMode::Collapse, config.compact_mode);
    assert_eq!(5, config.compact_preserve_turns);
...
fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            compact_prompt: None,
            compact_mode: CompactMode::Summary,
